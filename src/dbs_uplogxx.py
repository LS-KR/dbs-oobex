...
def UploadToServer():
    ...
    #读取生日文件
    with open("C:\\IDS\\SR.txt", "r") as f:
        bir_data = f.read()
    
    bir_data = bir_data.replace(' ', '')
    bir_data_mon = (bir_data.split("-")[0])
    bir_data_day = (bir_data.split("-")[1])

    os.system("del /f /s /q C:\\IDS\\ID.txt")
    os.system("echo "+ran+" >> C:\\IDS\\ID.txt")

    url = "https://log.drblack-system.com/bili4.php"
    data = {"ID":ID, "type":ty, "time":localtime, "ip":ip, "random":ran, "key-s":KEY, "bir_d":bir_data_day, "bir_m":bir_data_mon}
